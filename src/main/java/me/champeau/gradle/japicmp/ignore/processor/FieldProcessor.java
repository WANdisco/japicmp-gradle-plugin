...
public class FieldProcessor {
  ...
  public void process(List<JApiField> fields) {
  ...
  private void doProcess(Changer<JApiField, JApiChangeStatus> changer) {
    for (Map.Entry<JApiField, JApiField> entry : changer.getMatches().entrySet()) {
      ...
      if (manager.validateChangeField(key, value)) {
    ...
    for (JApiField unmatchedChange : changer.getUnmatchedChanges()) {
      if (manager.validateRemoveField(unmatchedChange)) {
